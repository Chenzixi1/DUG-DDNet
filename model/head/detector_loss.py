...
class Loss_Computation():
	...
	def prepare_predictions(self, targets_variables, predictions):
		...
		pred_regression_pois_3D = select_point_of_interest(batch, targets_bbox_points, pred_regression).view(-1, channel)[flatten_reg_mask_gt] # N x 50

		pred_regression_2D = F.relu(pred_regression_pois_3D[mask_regression_2D, self.key2channel('2d_dim')]) # N x 4
		pred_offset_3D = pred_regression_pois_3D[:, self.key2channel('3d_offset')] # N x 2
		pred_dimensions_offsets_3D = pred_regression_pois_3D[:, self.key2channel('3d_dim')] # N x 3
		pred_orientation_3D = torch.cat((pred_regression_pois_3D[:, self.key2channel('ori_cls')],
									pred_regression_pois_3D[:, self.key2channel('ori_offset')]), dim=1) # N x 16

		# decode the pred residual dimensions to real dimensions
		pred_dimensions_3D = self.anno_encoder.decode_dimension(target_clses, pred_dimensions_offsets_3D) # N x 3
