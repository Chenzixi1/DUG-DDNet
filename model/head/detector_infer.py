...
class PostProcessor(nn.Module):
	def __init__(self, cfg, anno_encoder, key2channel):
		...
		self.use_epistemic_uncertainty = cfg.TEST.USE_EPISTEMIC_UNCERTAINTY and self.testdropout
		if self.use_epistemic_uncertainty:
			self.key2channel.channels.append(1)
			self.key2channel.channels.append(20)
			self.key2channel.keys.append('depth_epistemic_uncertainty')
			self.key2channel.keys.append('corners_epistemic_uncertainty')
	...
	def forward(self, predictions, targets, features=None, test=False, refine_module=None):
		...
		if self.use_epistemic_uncertainty:
			depth_epistemic_uncertainty, corners_epistemic_uncertainty = predictions['depth_epistemic_uncertainty'], predictions['corners_epistemic_uncertainty']
			pred_regression = torch.cat([pred_regression, depth_epistemic_uncertainty, corners_epistemic_uncertainty], dim=1)
		...
		if self.depth_with_uncertainty:
			...
			if self.use_epistemic_uncertainty:
				depth_epistemic_uncertainty = pred_regression_pois[:, self.key2channel('depth_epistemic_uncertainty')].exp()
				pred_direct_uncertainty += depth_epistemic_uncertainty
				visualize_preds['depth_epistemic_uncertainty'] = pred_regression[:, self.key2channel('depth_epistemic_uncertainty'), ...].squeeze(1)
		...
		if self.keypoint_depth_with_uncertainty:
			...
			visualize_preds['corner_uncertainty'] = pred_regression[:, self.key2channel('corner_uncertainty'), ...].squeeze(1)
			if self.use_epistemic_uncertainty:
				corners_epistemic_uncertainty = pred_regression_pois[:, self.key2channel('corners_epistemic_uncertainty')]
				corners_epistemic_uncertainty = corners_epistemic_uncertainty.view(-1, 10, 2)

				# indirect corners epistemic uncertainty sum up
				center_height = corners_epistemic_uncertainty[:, -2, 1] + corners_epistemic_uncertainty[:, -1, 1]
				corner_02_height = corners_epistemic_uncertainty[:, [0, 2], 1] + corners_epistemic_uncertainty[:, [4, 6], 1]
				corner_13_height = corners_epistemic_uncertainty[:, [1, 3], 1] + corners_epistemic_uncertainty[:, [5, 7], 1]
				center_height = center_height.exp()
				corner_02_height = corner_02_height.exp()
				corner_13_height = corner_13_height.exp()

				corner_02_height = corner_02_height[:, 0] + corner_02_height[:, 1]
				corner_13_height = corner_13_height[:, 0] + corner_13_height[:, 1]
				corners_epistemic_uncertainty = torch.cat([center_height.unsqueeze(1), corner_02_height.unsqueeze(1), corner_13_height.unsqueeze(1)], dim=1)
				pred_keypoint_uncertainty += corners_epistemic_uncertainty

				vis_corners_epistemic_uncertainty = pred_regression[:, self.key2channel('corners_epistemic_uncertainty'), ...].squeeze(1)
				center_height = vis_corners_epistemic_uncertainty[:, -2, ...] + vis_corners_epistemic_uncertainty[:, -1, ...]
				corner_02_height = vis_corners_epistemic_uncertainty[:, 1, ...] + vis_corners_epistemic_uncertainty[:, 5, ...] + vis_corners_epistemic_uncertainty[:, 9, ...] + vis_corners_epistemic_uncertainty[:, 13, ...]
				corner_13_height = vis_corners_epistemic_uncertainty[:, 3, ...] + vis_corners_epistemic_uncertainty[:, 7, ...] + vis_corners_epistemic_uncertainty[:, 11, ...] + vis_corners_epistemic_uncertainty[:, 15, ...]
				vis_corners_epistemic_uncertainty = torch.stack([center_height, corner_02_height, corner_13_height], dim=1)
				visualize_preds['corners_epistemic_uncertainty'] = vis_corners_epistemic_uncertainty
