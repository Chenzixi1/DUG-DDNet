        ...
        for i, reg_feature_head in enumerate(self.reg_features):
            ...
            for j, reg_output_head in enumerate(self.reg_heads[i]):
                ...
                if self.enable_edge_fusion and i == self.offset_index[0] and j == self.offset_index[1]:
                    ...
                    # normalize
                    grid_edge_indices = edge_indices.view(b, -1, 1, 2).float() # B x K x 1 x 2
                    ...
                    feature_for_fusion = torch.cat((feature_cls, reg_feature), dim=1) # B x 512 x h x w
                    edge_features = F.grid_sample(feature_for_fusion, grid_edge_indices, align_corners=True).squeeze(-1) # B x 512 x K

                    edge_cls_feature = edge_features[:, :self.head_conv, ...] # B x 256 x K
                    edge_offset_feature = edge_features[:, self.head_conv:, ...] # B x 256 x K
                    edge_cls_output = self.trunc_heatmap_conv(edge_cls_feature) # B x C x K
                    edge_offset_output = self.trunc_offset_conv(edge_offset_feature) # B x 2 x K
                    
                    for k in range(b):
                        edge_indice_k = edge_indices[k, :edge_lens[k]] # K x 2
                        output_cls[k, :, edge_indice_k[:, 1], edge_indice_k[:, 0]] += edge_cls_output[k, :, :edge_lens[k]] # B x C x h x w
                        output_reg[k, :, edge_indice_k[:, 1], edge_indice_k[:, 0]] += edge_offset_output[k, :, :edge_lens[k]] # B x 2 x h x w
                
                output_regs.append(output_reg)

        output_cls = sigmoid_hm(output_cls) # B x 3 x h x w
        output_regs = torch.cat(output_regs, dim=1) # B x 50 x h x w
