...
def compute_on_dataset(model, data_loader, device, predict_folder, timer=None, vis=False,
                       use_depth=False, show=False, testdropout=False):
    model.eval()
    if testdropout:
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(data_loader)):
            images, targets, image_ids = batch["images"].to(device), batch["targets"], batch["img_ids"]
            depths = batch["depths"].to(device) if use_depth else None
            ...
            output, eval_utils, visualize_preds = model(images, depths=depths, targets=targets)
            ...
            if vis: show_image_with_boxes(vis_target.get_field('ori_img'), output, vis_target,
                                          visualize_preds, idx=image_ids[0], vis_scores=eval_utils['vis_scores'],
                                          outputpath=predict_folder[:-4], show=show)
...
def inference(
    ...
    dis_ious = compute_on_dataset(model, data_loader, device, predict_folder,
                                  inference_timer, vis, use_depth=use_depth, show=show, testdropout=testdropout)
    comm.synchronize()
...
def inference_all_depths(
    ...
    eval_depth_methods = ['oracle', 'hard', 'soft', 'mean', 'direct', 'keypoints_center', 'keypoints_02',
                          'keypoints_13']
    ...
    for cls_idx, cls in enumerate(important_classes):
        ...
        for thresh in cls_thresh:
            ...
            for depth_method, eval_depth_dict in zip(eval_depth_methods, eval_depth_dicts):
                ...
                logger.info('{:.4f}/{:.4f}, {:.4f}/{:.4f}, {:.4f}/{:.4f}'.format(
                    eval_depth_dict['{}_bev_{:.2f}/easy'.format(cls, thresh)],
                    eval_depth_dict['{}_3d_{:.2f}/easy'.format(cls, thresh)],
                    eval_depth_dict['{}_bev_{:.2f}/moderate'.format(cls, thresh)],
                    eval_depth_dict['{}_3d_{:.2f}/moderate'.format(cls, thresh)],
                    eval_depth_dict['{}_bev_{:.2f}/hard'.format(cls, thresh)],
