...
def do_eval(cfg, model, data_loaders_val, iteration):
	...
	is_test = True if cfg.DATASETS.TEST_SPLIT == 'test' else False
	vis = False if cfg.DATASETS.TEST_SPLIT == 'test' else True
	evaluate_metric, result_str, dis_ious = inference(
		...
		vis=vis,
		use_depth=cfg.DATASETS.USE_DEPTH_IMAGE,
		test=is_test
	)
	comm.synchronize()
...
def do_train(
	...
	for data, iteration in zip(data_loader, range(start_iter, max_iter)):
		...
		depths = data["depths"].to(device) if use_depth else None
		targets = [target.to(device) for target in data["targets"]]

		loss_dict, log_loss_dict = model(images, depths=depths, targets=targets)
		...
		if iteration % cfg.SOLVER.EVAL_INTERVAL == 0 and (cur_epoch == 1 or cur_epoch > 80):
			...
			if cfg.DATASETS.TEST_SPLIT == 'test':
				checkpointer.save("model_{}".format(cur_epoch), **arguments)
			else:
				if comm.get_rank() == 0:
