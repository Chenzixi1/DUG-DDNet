...
class KITTIDataset(Dataset):
	def __init__(self, cfg, root, is_train=True, transforms=None, augment=True):
		...
		self.depth_dir = os.path.join(root, "depth_2")
		...
		# whether to use depth-view image
		self.use_depth_img = cfg.DATASETS.USE_DEPTH_IMAGE
	...
	def get_depth(self, idx):
		img_filename = os.path.join(self.depth_dir, self.image_files[idx])
		img = Image.open(img_filename).convert('F')
		return img
	...
	def pad_depth(self, image):
		img = np.array(image)
		h, w = img.shape
		ret_img = np.zeros((self.input_height, self.input_width))
		...
		return Image.fromarray(ret_img.astype(np.float32)), pad_size
	...
	def __getitem__(self, idx):
		...
		if self.use_depth_img:
			dep = self.get_depth(idx)

		original_idx = self.image_files[idx][:6]

		objs = None if self.split == 'test' else self.filtrate_objects(objs) # remove objects of irrelevant classes
		 
		# random horizontal flip
		if self.augmentation is not None:
			if self.use_depth_img:
				img, objs, calib, dep = self.augmentation(img, objs, calib, dep)
			else:
				img, objs, calib, _ = self.augmentation(img, objs, calib, None)

		# pad image
		img_before_aug_pad = np.array(img).copy()
		if self.use_depth_img: dep_before_agu_pad = np.array(dep).copy()
		img_w, img_h = img.size
		img, pad_size = self.pad_image(img)
		if self.use_depth_img: dep, pad_size = self.pad_depth(dep)
		# for training visualize, use the padded images
		ori_img = np.array(img).copy() if self.is_train else img_before_aug_pad
		if self.use_depth_img: ori_dep = np.array(dep).copy() if self.is_train else dep_before_agu_pad
		...
		if self.split == 'test':
			...
			if self.use_depth_img: target.add_field("ori_dep", ori_dep)
			if self.enable_edge_fusion:
			...
			if self.transforms is not None:
				if self.use_depth_img:
					img, target, dep = self.transforms(img, target, dep)
				else:
					img, target, _ = self.transforms(img, target, None)

			if self.use_depth_img:
				return img, target, original_idx, dep
			else:
				return img, target, original_idx, _
		...
		if self.transforms is not None:
			if self.use_depth_img: img, target, dep = self.transforms(img, target, dep)
			else: img, target, _ = self.transforms(img, target, None)

		if self.use_depth_img: return img, target, original_idx, dep
		else: return img, target, original_idx, _
